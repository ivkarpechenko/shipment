...
readonly class BulkCreateShipmentCommandHandler implements CommandHandler
{
    ...
    public function __invoke(BulkCreateShipmentCommand $command): array
    {
        $createShipmentDto = $command->getBulkCreateShipmentDto();

        $this->logger->info(json_encode($createShipmentDto, JSON_UNESCAPED_UNICODE));
        ...
        if (empty($stores)) {
            ...
            throw new StoreProductNotFoundException(sprintf(
                'Error when creating stores with products, recipient address %s',
                $toAddress->getAddress()
            ));
        }
        ...
        foreach ($stores as $key => $store) {
            list($externalId, $deliveryPeriod) = explode(':', $key);

            try {
                $storeProducts = $this->queryBus->handle(new FindProductByStoreAndDeliveryPeriodQuery(
                    $store->getId(),
                    $deliveryPeriod
                ));
