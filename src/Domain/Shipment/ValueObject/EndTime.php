...
final readonly class EndTime
{
    ...
    public function equals(EndTime $startTime): bool
    {
    ...
    private function validate(string $value): void
    {
        ...
        if (strlen($value) !== self::LENGTH) {
            throw new StoreScheduleInvalidTimeFormatException('Invalid end time format.');
        }

        $datetime = \DateTime::createFromFormat(self::FORMAT, $value);
        if (!$datetime && $datetime->format(self::FORMAT) == $value) {
            throw new StoreScheduleInvalidTimeFormatException('Store schedule invalid end time format');
        }
    }
}
