...
class RestoreCityControllerTest extends HttpTestCase
{
    public function testRestoreCityRoute()
    {
        ...
        $newCity = CityFixture::getOneForDeleted($region, 'city', 'Moskva');
        $cityRepository->create($newCity);

        $this->client->request(
            'POST',
            "/api/v1/city/{$newCity->getId()->toRfc4122()}/restore"
        );

        self::assertResponseIsSuccessful();

        $restoredCity = $cityRepository->ofId($newCity->getId());

        $this->assertNotNull($restoredCity);
        $this->assertInstanceOf(City::class, $restoredCity);
        $this->assertEquals('Moskva', $restoredCity->getName());
        $this->assertEquals('city', $restoredCity->getType());
        $this->assertNotNull($restoredCity->getCreatedAt());
