...
use App\Application\Tax\Query\GetTaxesByPaginateQuery;
use App\Application\Tax\Query\GetTaxesByPaginateQueryHandler;
...
class GetTaxesByPaginateQueryTest extends MessageBusTestCase
{
    public function testQueryInstanceOf()
    {
        $this->assertInstanceOf(
            Query::class,
            new GetTaxesByPaginateQuery(1, 1)
        );
        $this->assertInstanceOf(
            QueryHandler::class,
            $this->getContainer()->get(GetTaxesByPaginateQueryHandler::class)
        );
    }
    ...
    public function testGetCountriesByPaginateQueryHandler()
    {
        ...
        $taxs = $container->get(GetTaxesByPaginateQueryHandler::class)(
            new GetTaxesByPaginateQuery(0, 2)
        );

        $this->assertNotEmpty($taxs);
        $this->assertIsArray($taxs);
        $this->assertArrayHasKey('data', $taxs);
        $this->assertArrayHasKey('total', $taxs);
        $this->assertArrayHasKey('pages', $taxs);
        ...
        $this->assertEquals(2, $taxs['total']);
        $this->assertEquals(1, $taxs['pages']);
    }
}
