...
use App\Application\DeliveryService\Command\CreateDeliveryServiceRestrictPackageCommandHandler;
...
class CreateDeliveryServiceRestrictPackageCommandTest extends MessageBusTestCase
{
    public function testCommandInstanceOf(): void
    {
        $this->assertInstanceOf(
            Command::class,
            new CreateDeliveryServiceRestrictPackageCommand(Uuid::v1(), 10, 20, 30, 40)
        );
        $this->assertInstanceOf(
            CommandHandler::class,
            $this->getContainer()->get(CreateDeliveryServiceRestrictPackageCommandHandler::class)
        );
    }

    public function testCommandDispatch(): void
    {
        $command = new CreateDeliveryServiceRestrictPackageCommand(Uuid::v1(), 10, 20, 30, 40);
        $this->commandBus->dispatch($command);

        $this->assertSame($command, $this->messageBus->lastDispatchedCommand());
    }

    public function testCreateDeliveryServiceRestrictPackageCommandHandler(): void
    {
        ...
        $container->get(CreateDeliveryServiceRestrictPackageCommandHandler::class)(
            new CreateDeliveryServiceRestrictPackageCommand($newDeliveryService->getId(), 10, 20, 30, 40)
        );

        $deliveryServiceRestrictPackage = $deliveryServiceRestrictPackageRepository->ofDeliveryServiceId($newDeliveryService->getId());
