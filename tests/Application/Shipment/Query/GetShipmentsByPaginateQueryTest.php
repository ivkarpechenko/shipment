...
use App\Application\Shipment\Query\GetShipmentsByPaginateQuery;
use App\Application\Shipment\Query\GetShipmentsByPaginateQueryHandler;
...
class GetShipmentsByPaginateQueryTest extends MessengerCommandBusTest
{
    public function testQueryInstanceOf()
    {
        $this->assertInstanceOf(
            Query::class,
            new GetShipmentsByPaginateQuery(1, 1)
        );
        $this->assertInstanceOf(
            QueryHandler::class,
            $this->getContainer()->get(GetShipmentsByPaginateQueryHandler::class)
        );
    }

    public function testGetShipmentsByPaginateQueryHandler()
    {
        ...
        $shipments = $container->get(GetShipmentsByPaginateQueryHandler::class)(
            new GetShipmentsByPaginateQuery(0, 1)
        );

        $this->assertNotEmpty($shipments);
        $this->assertIsArray($shipments);
        $this->assertArrayHasKey('data', $shipments);
        $this->assertArrayHasKey('total', $shipments);
        $this->assertArrayHasKey('pages', $shipments);
        $this->assertCount(1, $shipments['data']);

        $shipment = reset($shipments['data']);
