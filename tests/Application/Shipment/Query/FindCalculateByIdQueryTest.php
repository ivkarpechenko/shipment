...
use App\Application\Shipment\Query\FindCalculateByIdQueryHandler;
...
class FindCalculateByIdQueryTest extends MessageBusTestCase
{
    public function testQueryInstanceOf()
    {
        $this->assertInstanceOf(
            Query::class,
            new FindCalculateByIdQuery(Uuid::v1())
        );
        $this->assertInstanceOf(
            QueryHandler::class,
            $this->getContainer()->get(FindCalculateByIdQueryHandler::class)
        );
    }

    public function testFindCalculateByIdQueryHandler()
    {
        ...
        $newCalculate = CalculateFixture::getOneFilled($shipment, $tariffPlan, 1, 1, 100.0, 120.0, 20.0);
        $container->get(CalculateRepositoryInterface::class)->create($newCalculate);

        $calculate = $container->get(FindCalculateByIdQueryHandler::class)(
            new FindCalculateByIdQuery($newCalculate->getId())
        );
